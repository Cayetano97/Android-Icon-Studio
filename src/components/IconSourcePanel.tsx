...
const ALL_ICON_NAMES = Object.keys((LucideIcons as any).icons || LucideIcons).filter(name => {
  // Only include PascalCase names, exclude those ending with 'Icon' (redundant aliases),
  // and exclude known non-icon exports.
  return /^[A-Z][a-zA-Z0-9]+$/.test(name) && 
         !name.endsWith('Icon') && 
         !['LucideIcon', 'LucideProps', 'Lucide', 'default', 'createLucideIcon'].includes(name) &&
         // Additional check to ensure it's a component-like thing
         ((typeof (LucideIcons as any)[name] === 'function') || 
          (typeof (LucideIcons as any)[name] === 'object' && (LucideIcons as any)[name]?.render));
}).sort();
...
export default function IconSourcePanel({ config, onChange }: Props) {
  ...
  const selectedFont = useMemo(() => 
    SUPPORTED_FONTS.find(f => f.family === config.fontFamily) || SUPPORTED_FONTS[0],
    [config.fontFamily]
  );
  ...
  const handleFontFamilyChange = (family: string) => {
    const font = SUPPORTED_FONTS.find(f => f.family === family) || SUPPORTED_FONTS[0];
    ...
    if (!font.weights.includes(newWeight)) {
      newWeight = font.weights.reduce((prev, curr) => 
        Math.abs(curr - config.fontWeight) < Math.abs(prev - config.fontWeight) ? curr : prev
      );
    }
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <TabsList className="w-full grid grid-cols-3 mb-3">
          <TabsTrigger value="clipart" className="text-xs uppercase tracking-tight font-bold">Icons</TabsTrigger>
          <TabsTrigger value="text" className="text-xs uppercase tracking-tight font-bold">Text</TabsTrigger>
          <TabsTrigger value="image" className="text-xs uppercase tracking-tight font-bold">Image</TabsTrigger>
        </TabsList>

        <TabsContent value="clipart" className="flex flex-col outline-none mt-0">
        ...
        <TabsContent value="text" className="space-y-4 outline-none mt-0">
          <div className="bg-card/30 backdrop-blur-sm border border-border/40 p-5 rounded-2xl space-y-4">
            <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 block">Icon Text</label>
            <Input
              value={config.text}
            ...
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-muted-foreground/50 uppercase tracking-tighter">Maximum 5 characters</p>
              <span className="text-[10px] font-mono text-primary/60">{config.text.length}/5</span>
            </div>
          </div>

          <div className="bg-card/30 backdrop-blur-sm border border-border/40 p-5 rounded-2xl space-y-4">
            <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 block">Font</label>
            <div className="grid grid-cols-2 gap-2 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin">
              {SUPPORTED_FONTS.map((font) => (
                ...
                >
                  ...
                  <span className="text-[8px] uppercase tracking-widest opacity-50">{font.category}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-card/30 backdrop-blur-sm border border-border/40 p-5 rounded-2xl space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">Font Weight</label>
              <span className="text-xs font-bold text-primary">{config.fontWeight}</span>
            </div>
            ...
            {selectedFont.weights.length > 1 ? (
              <div className="space-y-4">
                <input
                  ...
                  onChange={(e) => onChange({ fontWeight: selectedFont.weights[parseInt(e.target.value)] })}
                ...
                <div className="flex justify-between px-1">
                  {selectedFont.weights.map(w => (
                    <span key={w} className={`text-[8px] font-mono ${config.fontWeight === w ? 'text-primary font-bold' : 'text-muted-foreground/40'}`}>
                      {w}
                    </span>
                  ))}
                </div>
              ...
              <div className="text-center py-2 bg-background/20 rounded-lg">
                <span className="text-[10px] text-muted-foreground italic">Fixed weight for this font</span>
              </div>
            )}
          </div>
        </TabsContent>
            ...
            {config.imageDataUrl ? (
              <div className="flex flex-col items-center gap-6">
                <div className="relative group">
                  ...
                  <img src={config.imageDataUrl} alt="Preview" className="relative w-32 h-32 object-contain rounded-2xl border border-border/40 bg-background/40" />
                </div>
                <button 
              ...
              >
                <div className="w-16 h-16 rounded-3xl bg-background/40 border border-border/40 flex items-center justify-center group-hover:scale-110 group-hover:border-primary/50 transition-all shadow-xl shadow-black/5">
                  <LucideIcons.Upload size={28} className="group-hover:translate-y-[-2px] transition-transform" />
                </div>
                <div className="text-center">
                  <span className="text-[10px] font-bold uppercase tracking-widest block mb-1">Upload image</span>
                  <span className="text-[9px] opacity-50">SVG, PNG or JPG supported</span>
                </div>
              </button>
            )}
          </div>
